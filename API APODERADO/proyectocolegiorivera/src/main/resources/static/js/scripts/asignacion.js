    ...
    $.ajax({
        ...
        success: function (data) {
			if(data==0){
	            ...
	            let texto = bandera == 1 ? "agregado": "actualizado";
	            getTabla();
	            Swal.fire({
	                icon: 'success',
	                title: 'Se ha '+texto+' la asignación al docente',
	                showConfirmButton: false,
	                timer: 1500
	            })
	            clear();
...
function deleteFila(id) {
    $.ajax({
        ...
        success: function (data) {
            Swal.fire({
                icon: 'success',
                title: 'Se ha eliminado la asignacion al docente',
                showConfirmButton: false,
                timer: 1500
            });
            getTabla();
...
function getTabla() {
    $.ajax({
        ...
        success: function (data) {
		    ...
		    //concatenación tabla docente		            
            let docenteNombre 	      = b.docente.nombre;
            let docenteApPaterno      = b.docente.apellidopaterno;
            let docenteApMaterno      = b.docente.apellidomaterno;   
            let nombreCompletoDocente = docenteNombre + " " + docenteApPaterno + " " + docenteApMaterno;
            
            //concatenación tabla horario
            let horarioDia 		  = b.horario.dia;
            let horarioHoraInicio = b.horario.horaInicio;  
            let horarioHorafinal  = b.horario.horaFinal; 
            let horarioCompleto   = horarioDia + " de  " + horarioHoraInicio + " a " + horarioHorafinal;
            
            t.row.add ([b.id, nombreCompletoDocente , b.aula.numero, b.curso.nombre, horarioCompleto, botonera]);
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                $("#docente_id").append("<option value='" + id + "'>" + nombre +" "+ apellidopaterno+" "+ apellidomaterno +"</option>");
            }
        },
    }).fail(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                var id = data[i]['id'];
                var dia = data[i]['dia'];
                var horaInicio = data[i]['horaInicio'];
                var horaFinal = data[i]['horaFinal'];
                $("#horario_id").append("<option value='" + id + "'>" + dia +" de "+ horaInicio +" a "+ horaFinal +"</option>");
            }
        },
    }).fail(function () {
